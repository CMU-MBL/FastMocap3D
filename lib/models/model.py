...
from .layers import MultiHeadSelfAttentionBlock, MultiHeadAttentionBlock
...
class Network(nn.Module):
    def __init__(self, args, mods=[]):
        ...
        self.residual_regress = args.residual_regress
        ...
        for mod in mods:
            ...
            n_mod = int(args.joint_type[-2:]) if mod == 'video' else len(_C.SENSOR_LIST)
        ...
        expansion = 2 if self.aggregate_method == 'cat' else 1
        self._make_attn_modules('fusion', 'temporal', l_mod, d_emb * expansion, 'self')
        self._make_decoder_layers('fusion', d_emb * expansion, n_smpl)
    ...
    def _get_unimodal_regression(self, x, modality, attn_dict={}):
        # Dimensions
        b, f, m = x.shape[:3]; s = 22
        ...
        x, attns = self.forward_attn(x, modality, 'spatial')
        attn_dict[modality + '_spatial'] = attns

        # Temporal self attention
        x = rearrange(x, '(b f) m d -> (b m) f d', b=b, f=f)
        x, attns = self.forward_attn(x, modality, 'temporal')
        attn_dict[modality + '_temporal'] = attns
        ...
        pose = transforms.rotation_6d_to_matrix(pose).contiguous().view(-1, 22, 3, 3)
    ...
    def _get_multimodal_regression(self, x_video, x_imu, pose_b):
        ...
        x_VtoI, attns = self.forward_attn(x_imu, 'VtoI', 'spatial', x_video)
        x_ItoV, attns = self.forward_attn(x_video, 'ItoV', 'spatial', x_imu)
        ...
        x_VtoI, attns = self.forward_attn(x_imu, 'VtoI', 'temporal', x_video)
        x_ItoV, attns = self.forward_attn(x_video, 'ItoV', 'temporal', x_imu)

        # Aggregate modalities
        if self.aggregate_method == 'cat':
            x = torch.cat((x_ItoV, x_VtoI), dim=-1)
        elif self.aggregate_method == 'max':
            x = torch.stack((x_ItoV, x_VtoI)).max(0)[0]
        elif self.aggregate_method == 'mean':
            x = torch.stack((x_ItoV, x_VtoI)).mean(0)

        # Temporal Self Attention
        x, attns = self.forward_attn(x, 'fusion', 'temporal')
        ...
        pose = rearrange(pose, '(b f) s d -> b f s d', b=b)
        pose = transforms.rotation_6d_to_matrix(pose)
        ...
        return pose, shape, transl


    def forward_video(self, inputs):
        init_transl = inputs['keypoints'][:, :, 0]
        input_video = inputs['keypoints'] - init_transl.unsqueeze(-2)
    ...
    def forward_fusion(self, inputs):
        ...
        if self.residual_regress == 'video':
            pose_b = pose_v
        elif self.residual_regress == 'imu':
            pose_b = pose_i

        pose_b = pose_b[..., :-1].contiguous().reshape(*pose_b.shape[:3], 6)
        pose, shape, transl = self._get_multimodal_regression(x_video, x_imu, pose_b)
        transl = transl + init_transl
